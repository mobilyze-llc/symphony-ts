...
import {
  type TrackFindingIssueInput,
  type TrackFindingIssueResult,
  createTrackFindingIssue as createTrackFindingIssueWithProvenance,
  findPreferredIntakeStateId,
} from "./track-finding-intake.js";
...
export class LinearTrackerClient implements IssueTracker {
  ...
  async createWatchdogIssue(input: {
    ...
    const states = statesResponse.workflowStates?.nodes ?? [];
    const targetStateId = findPreferredIntakeStateId(states);
   ...
   * duplicate. A disposition-cancelled twin also suppresses a refile when its
   * stable triage marker and superseded-by relation point to a surviving open
   * root. Otherwise resolves a Triage (then Backlog) state and creates the issue.
   * Triage is intake and is deliberately inert until an operator rubric pass
   * promotes the finding to Backlog.
   *
   ...
   */
  async createTrackFindingIssue(
    input: TrackFindingIssueInput,
  ): Promise<TrackFindingIssueResult> {
    return createTrackFindingIssueWithProvenance(input, {
      pageSize: this.pageSize,
      workflowStatesQuery: LINEAR_WORKFLOW_STATES_QUERY,
      postGraphql: <T>(query: string, variables: Record<string, unknown>) =>
        this.postGraphql<T>(query, variables),
      postComment: (issueId, body) => this.postComment(issueId, body),
    });
  }
