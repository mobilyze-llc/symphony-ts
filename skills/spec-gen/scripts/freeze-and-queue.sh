...
# Globals populated by resolve_spec_label():
SPEC_LABEL_ID=""
SPEC_LABEL_IDS_GQL="[]"

resolve_spec_label() {
  local labels_json
  labels_json=$(run_with_timeout "resolving Spec label" $LINEAR_CLI api \
    'query { issueLabels(filter: { name: { eq: "Spec" } }) { nodes { id name } } }')

  SPEC_LABEL_ID=$(echo "$labels_json" | jq -r '.data.issueLabels.nodes[] | select(.name == "Spec") | .id' | head -1)

  if [[ -n "$SPEC_LABEL_ID" ]]; then
    SPEC_LABEL_IDS_GQL="[\"${SPEC_LABEL_ID}\"]"
    echo "Spec label: found (ID: $SPEC_LABEL_ID)"
  else
    SPEC_LABEL_IDS_GQL="[]"
    echo "WARNING: 'Spec' workspace label not found. Parent issue will not be labeled." >&2
  fi
}
