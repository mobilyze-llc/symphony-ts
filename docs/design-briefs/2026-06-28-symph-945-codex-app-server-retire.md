...
4. **Cut over per stage** (review is already done): merge → investigate → implement, each after a shadow window. **Rationale for the order:** merge is the single-turn, lowest-traffic canary (it validates wiring); investigate/implement are multi-turn and must not be cut over until step 2 is resolved — merge-first is *not* evidence the multi-turn contract works.
