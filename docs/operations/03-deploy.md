...
- **`symphony-ctl install` refuses a worktree root.** Running `install` from a `.claude/worktrees/...` or any linked git worktree dies (`Refusing to install from worktree root`). Re-run from the stable checkout, or `SYMPHONY_ALLOW_WORKTREE_ROOT=1` to override.
