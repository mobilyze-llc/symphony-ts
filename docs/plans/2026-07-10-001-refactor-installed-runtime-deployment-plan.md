...
- R3. `${HOME}/.local/share/symphony/active` changes atomically between immutable activation bundles. Each bundle resolves exactly one validated release and its compatible external configuration generation, so launchers cannot observe a mixed pair.
...
- **Installed link or wrapper is replaced:** Validate ownership, modes, regular-file type, and realpath containment of `active`, activation-bundle entries, and stable launchers before activation and start.
