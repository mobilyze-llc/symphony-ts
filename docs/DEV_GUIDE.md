...
For SYMPH/MOB issue project assignment, use `symphony-linear-portfolio` instead of raw `linear_graphql` `issueCreate`, `issueUpdate`, or `issueBatchUpdate` project writes. The wrapper classifies the issue into the portfolio taxonomy, stamps a `## Portfolio Classification` block into the description, and routes ambiguous issues to `Portfolio Intake / Needs Classification` with `why_uncertain` and candidate projects. Raw `linear_graphql` rejects direct `projectId` writes on issue create/update/batch-update so project metadata cannot bypass classification.
