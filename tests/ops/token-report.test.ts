...
const EXTRACT_COMMAND_TIMEOUT_MS = 30000;
const EXTRACT_TEST_TIMEOUT_MS = 30000;
...
describe("token-report.mjs extract", () => {
  ...
  it(
    "extracts token history from fresh logs across 2 products",
    () => {
    ...
    EXTRACT_TEST_TIMEOUT_MS,
  );

  it(
    "extracts both completed and failed stages",
    () => {
  ...
  it(
    "idempotent re-extraction produces no duplicates",
    () => {
  ...
  it(
    "handles HWM recovery after file truncation",
    () => {
      const dir = join(logDir, "prod");
      mkdirSync(dir, { recursive: true });
  ...
  it(
    "discards partial line at EOF during active writing",
    () => {
      const dir = join(logDir, "prod");
      mkdirSync(dir, { recursive: true });
      const logPath = join(dir, "symphony.jsonl");
  ...
  it(
    "skips malformed JSONL lines without failing",
    () => {
      const dir = join(logDir, "prod");
      mkdirSync(dir, { recursive: true });
      const logPath = join(dir, "symphony.jsonl");
  ...
  it(
    "graceful degradation without Linear auth",
    () => {
      const dir = join(logDir, "prod");
      mkdirSync(dir, { recursive: true });
      writeFileSync(
